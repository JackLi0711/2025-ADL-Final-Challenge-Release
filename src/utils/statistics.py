...
class StatisticsAnalyzer:
    ...
    def _plot_safety(self, records: List[RewriteRecord]) -> Path:
        ...
        2. 說明: 以長條圖展示各 category 在 safety_score 0、0.5、1 的分佈。
        3. return: 產出的圖檔路徑。
        """
    ...
    def _plot_relevance(self, records: List[RewriteRecord]) -> Path:
        """
        1. inputs: records (RewriteRecord 列表)
        2. 說明: 以長條圖展示各 category 在 relevance_score 0、1 的分佈。
        3. return: 產出的圖檔路徑。
        """
        categories = self._category_order or sorted({r.category for r in records})
        ...
        plt.xlabel("Category")
        plt.ylabel("Count")
        plt.title("Relevance Score Distribution by Category")
        plt.xticks(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--original",
        type=Path,
        default=Path("data/prompts_ADL_Final_25W_part1_with_cost.jsonl"),
        help="原始 JSONL 檔案路徑。",
    )
    parser.add_argument(
        "--rewrite",
        type=Path,
        default=Path("results/algorithm_asr/raw_ADL_Final_25W_part1_with_cost.jsonl"),
...
python src/utils/statistics.py \
    --original data/prompts_ADL_Final_25W_part1_with_cost.jsonl \
    --rewrite results/algorithm_asr/raw_ADL_Final_25W_part1_with_cost.jsonl
"""
